...
class SubCategoryManagementPage
    extends GenericManagementPageView<SubCategory, SubCategoryModel> {
  ...
  Widget? builderContainer(
    ...
    CommonResult<List<SubCategory?>?> data,
  ) {
    return _list(context, data);
  }

  Widget _cardInfo(BuildContext context, List<SubCategory?> categories) {
    return Card(
      elevation: 4,
      child: ListTile(
        title: const Text(
  ...
  Widget _list(
    BuildContext context,
    CommonResult<List<SubCategory?>?> categories,
  ) {
    return Column(
      children: [
        Row(
          ...
          children: [
            Expanded(
              child: ServiceLocator.instance
                  ...
                  .createlListView(subcategories: categories.data),
            ),
          ],
        ),
      ],
